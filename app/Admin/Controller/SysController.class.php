...
class SysController extends AuthController {
	...
	$CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./public/ueditor/config.json")), true);//加载配置项
	$action = I('action');		
	switch ($action) {
		case 'config':
	...
	/* 输出结果 */
	if (isset(I('callback')) && false ) {//TODO 跨域上传
	...
	private function _ueditor_upload($config=array()){
		...
		if ($info) {
			...
			if(!empty($info['upfile']['url'])){
				...
				$url = C("TMPL_PARSE_STRING.__UPLOAD__").$info['upfile']['savepath'].$info['upfile']['savename'];
			}
